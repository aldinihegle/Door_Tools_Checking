<template>
  <div>
    <h2>Komparasi Potongan Koperasi</h2>
    ...
    <table v-if="comparison.length">
      <thead>
        <tr>
          <th>Kode</th>
          <th>Nama</th>
          <th>Potongan Koperasi Sekarang</th>
          <th>Potongan Koperasi Sebelumnya</th>
          <th>Selisih</th>
        </tr>
      </thead>
      <tbody>
        <tr v-for="row in comparison" :key="row.code">
          <td>{{ row.code }}</td>
          <td>{{ row.name }}</td>
          <td>{{ row.amount_now ?? 0 }}</td>
          <td>{{ row.amount_prev ?? 0 }}</td>
...
async function fetchComparison() {
  ...
  comparison.value = []
  try {
    const url = `/api/payroll/compare-koperasi-deduction?period_now=${periodNow.value}&period_prev=${periodPrev.value}`
